...
let step x = List.map (fun i -> if (i + x) >= 10 then (i + x) - 9 else (i + x))

let parse_input l =
  l
  |> List.map (str_to_char_list)
  |> List.map (List.map (fun x -> x |> int_of_char |> (-) 48 |> (~-)))
  |> List.map (fun x -> List.fold_left (fun acc t -> acc @ step t x) x [1;2;3;4])
  |> (fun x -> List.fold_left (fun acc t -> acc @ List.map(step t) x) x [1;2;3;4])
