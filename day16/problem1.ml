...
parse_and_rest l =
  ...
  if type_id = 4 then
    ...
    let sub_packets, rest = parse_operator [] rest in
    OperatorPacket { version; type_id; sub_packets}, rest

let rec version_sum = function
| LiteralPacket {version} ->
  version
| OperatorPacket {version; sub_packets} ->
  version + List.fold_left (fun sum x -> sum + version_sum x) 0 sub_packets
