...
f boards (x : xs)
  | null boardsRemainingAfterRound = f boardsAfterRound [x]
  | otherwise = f boardsRemainingAfterRound xs
  where
    boardsAfterRound = getBoardsAfterRound x boards
    boardsRemainingAfterRound = filter (not . checkIfBoardIsDone) boardsAfterRound
